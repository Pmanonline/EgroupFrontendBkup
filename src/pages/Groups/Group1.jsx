...
const EGroup = () => {
  ...
  const fetchGroupData = async () => {
    ...
    try {
      const response = await axios.get(
        `${backendURL}/api/groups/getGroupBySlug/${slug}`
      );
  ...
  const checkMembership = async () => {
    ...
    try {
      const response = await axios.post(
        `${backendURL}/api/groups/members/${groupId}`,
        { email }
      );
  ...
  const handleJoinGroup = async () => {
    ...
    try {
      await axios.post(`${backendURL}/api/groups/join/${groupId}`, {
        email: email,
        id: userId,
        name: name,
      });
  ...
  const handleLeaveGroup = async () => {
    ...
    setMembershipLoading(true);
    try {
      await axios.post(`${backendURL}/api/groups/leave/${groupId}`, {
        email: email,
      });
  ...
  const handleDiscussionSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `${backendURL}/api/discussions/createDiscussion`,
        {
    ...
    if (isGroupMember) {
      return (
        <div className="flex items-center gap-2">
          ...
          {groupData?.creator?.email !== email && (
            <button
              onClick={handleLeaveGroup}
              disabled={membershipLoading}
              className="flex items-center gap-1 text-xs bg-red-600 hover:bg-red-700 px-3 py-1 rounded-full text-white disabled:opacity-50">
              {membershipLoading ? "Leaving..." : "Exit Group"}
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* <div className="bg-red-100 rounded-xl text-black shadow-lg mb-8"> */}
      <div className="bg-gradient-to-r from-blue-300 via-red-100 to-pink-500 rounded-xl text-black shadow-lg mb-8">
