...
const GroupDiscussionPage = () => {
  ...
  const handleLikeDiscussion = async () => {
    if (!email) {
      console.error("Error: email is undefined before making request");
      showAlertMessage("Email is missing!", "destructive");
      return;
    }

    try {
      const response = await fetch(
        `${backendURL}/api/discussions/likeDiscussion`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            email: email,
            discussionId: discussion?._id, // Ensure discussionId is not undefined
          }),
        }
      );
  ...
  const handleCreateComment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${backendURL}/api/discussions/Createcomment/${discussion._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleCommentDelete = async (replyId) => {
    try {
      const response = await fetch(
        `${backendURL}/api/discussions/delete/${discussion._id}/comments/${replyId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
